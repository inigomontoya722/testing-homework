...
```sh
Для проверки unit тестов запускать:
npm run test:unit
С багом:
BUG_ID=10 npm run test:unit
Для проверки e2e тестов лучше запустить приложение через npm start и потом запускать npx hermione gui,
С багом запускать BUG_ID=10 npm start

Времени на тесты всего не хватило. Получилось отловить 9 из 10 багов. Тесты 5-10 отлавливаются в юнитах, а 1,2,4 в гермионе.
```
